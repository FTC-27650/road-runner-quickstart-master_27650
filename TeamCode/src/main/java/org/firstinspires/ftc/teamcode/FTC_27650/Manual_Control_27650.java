...
public class Manual_Control_27650 extends LinearOpMode {
    ...
    double n = 1;
    int a = 2;
    boolean mecaunm_1_Using = false;
    boolean mecaunm_2_Using = true;
    //**********收集器电机**********//
        ...
        while (opModeIsActive()) {
            ...
            if (gamepad2.left_bumper) {
                ...
                while (up_prepare_fang_gaoKuang_using && opModeIsActive()) {
                    if (gamepad2.left_bumper) {
                        up_prepare_fang_gaoKuang.start();
                    }
                    putong();
                }
            }
            ...
            if (gamepad2.left_trigger > 0.2) {
                ...
                while (up_prepare_gaoGan_down_using && opModeIsActive()) {
                    if (gamepad2.left_trigger > 0.2) {
                        up_prepare_gaoGan_down.start();
                    }
                    putong();
                }
            }
            if (gamepad2.right_stick_button) {
                up_prepare_fang_diKuang.start();
            }
            //普通方法//
            putong();
        }
    ...
    public void arm_and_put_servo() {
        ...
        if (gamepad1.right_trigger > 0.2) {
            forward_arm_servo_position = 0.685;
        }
